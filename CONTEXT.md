...
| **settings change** | The settings value changing, from the panel's fields or from `omarchy bar set`. One rule for both: a fresh block adopts the new length, a paused one keeps its remaining time, a running one keeps its deadline. | `Block.js` |
